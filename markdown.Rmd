...
for(j in 1:k){
  ...
  k_fold <- subset(numeric_cols_sing_rm,subset=k_out,select=c(-k_indices,-train_ind,-a1_all_d,-REGION.x,-REGION.y,-a1_all_docs_n,-a1_all_abs_n,-a1_all_oth_n,-a1_all_abs_r,-a1_all_oth_r))
  k_fold_test <- subset(numeric_cols_sing_rm,subset=k_in,select=c(-k_indices,-train_ind,-a1_all_d,-REGION.y))
...
Forward and backward achieved the best at 36 variables with MSE 19314
```{r}
...
```{r}
sub_model_build <- subset(numeric_cols_sing_rm,select=c(-k_indices,-train_ind,-a1_all_d,-REGION.x,-REGION.y,-a1_all_docs_n,-a1_all_abs_n,-a1_all_oth_n,-a1_all_abs_r,-a1_all_oth_r))
reg.best=regsubsets (a1_all_rs**-1.39∼.,data=sub_model_build , nvmax =70,method=c('seqrep'))
coef(reg.best,36)
select_var <- names(coef(reg.best,36))[-1]
```
...
for(j in 1:k){
  ...
  weights <- numeric_cols_sing_rm[k_out,c("a1_all_d")]
...
for(j in 1:k){
  k_out <- (numeric_cols_sing_rm$k_indices != j)
  k_in <- (numeric_cols_sing_rm$k_indices == j)
  k_fold <- subset(numeric_cols_sing_rm,subset=k_out,select=c(-k_indices,-train_ind,-a1_all_d,-REGION.x,-REGION.y,-a1_all_docs_n,-a1_all_abs_n,-a1_all_oth_n,-a1_all_abs_r,-a1_all_oth_r,-r_all_rs,-w_all_rs,-DZCAMPUS,-DPETBLAP,-DPETHISP,-DPETWHIP,-DPETINDP,
-DPETASIP,-DPETPCIP,-DPETTWOP,-DPETBILP,-DPSTTOFC,-DPFEAOPFT,-DPFEAINST,
-DPETALLC,-DPETECOP,-DPETLEPP,-DPSATOFC,-DPSAMIFP))
  k_fold_test <- subset(numeric_cols_sing_rm,subset=k_in,select=c(-k_indices,-train_ind,-REGION.y))
  weights <- numeric_cols_sing_rm[k_out,c("a1_all_d")]
...
#MSE 
Foward , 21614.35 MSE. The diagnostic plot shows a strong outlier.
Sequential, 21380.49
```{r}
...
```{r}
sub_model_build <- subset(numeric_cols_sing_rm,select=c(-k_indices,-train_ind,-a1_all_d,-REGION.x,-REGION.y,-a1_all_docs_n,-a1_all_abs_n,-a1_all_oth_n,-a1_all_abs_r,-a1_all_oth_r,-a1_all_abs_r,-a1_all_oth_r,-r_all_rs,-w_all_rs,-DZCAMPUS,-DPETBLAP,-DPETHISP,-DPETWHIP,-DPETINDP,
-DPETASIP,-DPETPCIP,-DPETTWOP,-DPETBILP,-DPSTTOFC,-DPFEAOPFT,-DPFEAINST,
-DPETALLC,-DPETECOP,-DPETLEPP,-DPSATOFC,-DPSAMIFP,-DPFRAALLT,-DPFEAALLT))
weights <- numeric_cols_sing_rm[,c("a1_all_d")] 
reg.best=regsubsets (a1_all_rs**-1.39∼.,data=sub_model_build , nvmax =70,method=c('forward'),weights=weights)
coef(reg.best,13)
select_var <- names(coef(reg.best,13))[-1]
```
...
-DPETASIP,-DPETPCIP,-DPETTWOP,-DPETBILP,-DPSTTOFC,-DPFEAOPFT,-DPFEAINST,
-DPETALLC,-DPETECOP,-DPETLEPP,-DPSATOFC,-DPSAMIFP))
#weights <- numeric_cols_sing_rm[,c("seqrep")] 
reg.best=regsubsets (a1_all_rs**-1.39∼.,data=sub_model_build , nvmax =70,method=c('seqrep'))
coef(reg.best,60)
select_var <- names(coef(reg.best,60))[-1]
```
...
#cv.glmnet
set.seed(1011)
X <- as.matrix(subset(numeric_cols_sing_rm,subset = (numeric_cols_sing_rm$train_ind == 1) ,select=c(-k_indices,-train_ind,-a1_all_d,-REGION.x,-REGION.y,-a1_all_docs_n,-a1_all_abs_n,-a1_all_oth_n,-a1_all_abs_r,-a1_all_oth_r -a1_all_rs)))
y <- numeric_cols_sing_rm[(numeric_cols_sing_rm$train_ind == 1),'a1_all_rs']**-1.39
#without weights
...
X_test <- X <- as.matrix(subset(numeric_cols_sing_rm,subset = (numeric_cols_sing_rm$train_ind == 0) ,select=c(-k_indices,-train_ind,-a1_all_d,-REGION.x,-REGION.y,-a1_all_docs_n,-a1_all_abs_n,-a1_all_oth_n,-a1_all_abs_r,-a1_all_oth_r -a1_all_rs)))
